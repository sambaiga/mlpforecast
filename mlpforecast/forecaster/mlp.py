...
class MLPForecast(PytorchForecast):
    ...
    def get_search_mlp_params(self, trial: Trial) -> dict:
        """
    ...
    def get_search_gam_params(self, trial: Trial) -> dict:
        """
        Define the search space for hyperparameter optimization using Optuna.
    ...
    def auto_tune(self, train_df, val_df, num_trial=10, reduction_factor=3, patience=2):
        ...
        def objective(trial):
            if self.hparams['gam_layer']:
                params = self.get_search_gam_params(trial)
            else:
                params = self.get_search_mlp_params(trial)
