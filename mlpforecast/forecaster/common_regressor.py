...
class BasicForecast:
    ...
    def fit(
        ...
        if self.trial is not None:
            ...
            cost = metrics.groupby("target")["MAE"].median().iloc[-1].round(2)
            return cost
        else:
    ...
    def evaluate(self, test_df=None, covariate_df=None, daily_feature=True):
        ...
        if(len(output["loc"])<len(ground_truth)):
            ground_truth = ground_truth[:len(output["loc"])]
            time_stamp = time_stamp[:len(output["loc"])]
        elif (len(ground_truth)<len(output["loc"])):
            output["loc"] = output["loc"][:len(ground_truth)]
