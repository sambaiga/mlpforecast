...
def visualise_timeseries_altair(data,  y_col, figure_path=None, y_label='Power (kW)'):
    ...
    """
    chart = alt.Chart(data.reset_index()).mark_point(filled=True, opacity=0.7).encode(
            x=alt.X('timestamp:T',
                    scale=alt.Scale(zero=False),
...
def plot_distribution(ax, df, index_col='HOUR', val_col='WindSpeed', hue_col=None):
    ...
    This function creates a pivot table to calculate the mean and standard deviation of `val_col`
    grouped by `index_col` and optionally by `hue_col`. It then plots the mean values with bands
