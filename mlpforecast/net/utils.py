...
def autoregressive_forecast(model, initial_hist_df, covariate_df, forecast_horizon, input_window_size, target_transformer, max_days=7):
    """
    ...
    historical_slices = [initial_hist_df[i:forecast_horizon+i].values for i in range(0, input_window_size, forecast_horizon)]
    ...
    with tqdm(total=max_days, file=sys.stdout) as pbar:
        for k in range(max_days):
            ...
            future_data = covariate_df.iloc[second_day_end:second_day_end + forecast_horizon]

            historical_target = np.concatenate([day_1, day_2], axis=0)
            unknown_features = np.concatenate([historical_target, historical_data.values], axis=1).astype(np.float64)
            ...
            covariates = torch.cat([torch.zeros(B, N, diff, requires_grad=False), covariates], dim=-1)
            ...
            out['pred'] = inverse_scaling(out['pred'], target_transformer).flatten()
