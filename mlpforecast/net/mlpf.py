...
def FeedForward(dim: int, expansion_factor: int=2, dropout: float=0.0, 
                activation: torch.nn.Module=nn.GELU(), bn:bool=True):
    """
...
class PastFutureEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.encoder = MLPBlock(
            in_size=embedding_size if embedding_type in ["PosEmb", "RotaryEmb", "CombinedEmb"] else n_channels,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.embedding_type in ["PosEmb", "RotaryEmb", "CombinedEmb"]:
...
class MLPForecastNetwork(nn.Module):
    ...
    def step(self, batch: tuple, metric_fn: callable) -> tuple:
        ...
        loss = (
            self.alpha * F.mse_loss(y_pred, y)
            + (1 - self.alpha) * F.l1_loss(y_pred, y)
        )
